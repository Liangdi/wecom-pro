...
通过 `wecom-pro` 查询企业微信待办列表，支持按时间过滤和分页。
...
```bash
wecom-pro todo get_todo_list '<json格式的入参>'
```

### 多 Bot 模式

```bash
wecom-pro todo --bot <bot_id> get_todo_list '<json格式的入参>'
```
...
```bash
# 使用 work bot 查询工作待办
wecom-pro todo --bot work get_todo_list '{}'

# 使用 personal bot 查询个人待办
wecom-pro todo --bot personal get_todo_list '{"remind_begin_time": "2026-04-01 00:00:00", "remind_end_time": "2026-04-30 23:59:59"}'
```
...
```bash
wecom-pro todo get_todo_detail '{"todo_id_list": ["返回的TODO_ID_1", "返回的TODO_ID_2"]}'
```
...
```bash
wecom-pro todo get_todo_list '{"create_begin_time": "2025-03-01 00:00:00", "create_end_time": "2025-03-31 23:59:59"}'
```
...
- 首次请求（不传 cursor）：
```bash
wecom-pro todo get_todo_list '{"limit": 20}'
```
，如果没有拉取完，还有更多的待办，会返回 has_more=true, next_cursor="CURSOR_1"
- 第二次请求（传入上次的 next_cursor）：
```bash
wecom-pro todo get_todo_list '{"limit": 20, "cursor": "CURSOR_1"}'
```
